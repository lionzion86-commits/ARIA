...
check("Hero, Ofertas, brand band, store rails, Todas las otras tiendas, Categorías", () => {
  ...
  eq(order.join(" > "), "Todo USA ahora en Lima > Ofertas > Compra en Estados Unidos > Aria Auto > Foot Locker > New Balance > Sephora > Macy's > Kohl's > Dick's Sporting Goods > Victoria's Secret > Marcas > Costco > B&H Photo > Gymshark > SSENSE > Fiestas y Eventos > Todas las otras tiendas > Categorías", "the home page's scroll order");
...
check("the desktop shopfront reads Ofertas, brand band, store rails, Todas las otras tiendas, Categorías", () => {
  ...
  eq(order.join(" > "), "Ofertas > Compra en Estados Unidos > Aria Auto > Foot Locker > New Balance > Sephora > Macy's > Kohl's > Dick's Sporting Goods > Victoria's Secret > Marcas > Costco > B&H Photo > Gymshark > SSENSE > Fiestas y Eventos > Todas las otras tiendas > Categorías", "the desktop shopfront's scroll order");
  // Each section owns exactly one rail, and the rails are the ids the
  // renderers write into.
...
check("the store rails reuse the page's own cards, feeds and registry", () => {
  ...
  /* The eleven window displays, reliable-first (2026-09-26, Danny): Foot
     ...
     shippers -- then Victoria's Secret, Costco, Gymshark, SSENSE. The
  ...
  eq(m[1].replace(/['\s]/g, ""), "footlocker,newbalance,sephora,macys,kohls,dicks,victoriassecret,costco,bhphoto,gymshark,ssense",
    "the store rails are not the eleven agreed stores in mall order");
...
check("the eleven rails stand in mall order on all three surfaces", () => {
  ...
  const src = HOME_SRC();
  /* RELIABLE-FIRST RAIL ORDER (2026-09-26, Danny): eleven rails, one
     order on the phone, the laptop, and the Tiendas vitrinas. */
  ...
  for (const [name, prefix, from, to] of [
    ...
    eq(found.join(","), want.join(","), `${name} does not carry the eleven rails in mall order`);
  }
});
